...
function hasEquivalentHeapSamplingConfig(intervalBytes, stackDepth) {
    if (intervalBytes === heapIntervalBytes &&
    ...
    return (Number(intervalBytes) === Number(heapIntervalBytes) &&
...
function start(intervalBytes, stackDepth) {
    if (enabled) {
        ...
        throw new Error(`Heap profiler is already started  with intervalBytes ${heapIntervalBytes} and stackDepth ${heapStackDepth}`);
    }
