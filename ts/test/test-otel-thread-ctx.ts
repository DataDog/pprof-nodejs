...
function decodeHeader(bytes: Uint8Array): Header {
  strictAssert.ok(bytes.length >= 28, `record must be at least 28 bytes, got ${bytes.length}`);
...
function decodeAttrs(bytes: Uint8Array): Array<string | undefined> {
  ...
  strictAssert.equal(i, end, 'attrs payload must be exactly attrsDataSize bytes');
  return out;
}
...
(isLinux ? describe : describe.skip)('OTEP-4947 thread context (Linux-only)', () => {
  ...
  describe('attribute encoding', () => {
    ...
    it('skip-and-continue truncates past the 612-byte cap', () => {
      ...
      runWithContext(
        ...
        {traceId: TRACE_ID_BYTES, spanId: SPAN_ID_BYTES, attributes: [a, b, c, d]},
      );
  ...
  describe('enterWithContext', () => {
    it('attaches the record to the current async scope', () => {
      runWithContext(() => {
        ...
        strictAssert.deepEqual(decodeHeader(_currentRecordBytes()!).spanId, newSpan);
  ...
  describe('appendAttributes', () => {
    it('adds entries to the current record', () => {
      runWithContext(
        () => {
          strictAssert.deepEqual(decodeAttrs(_currentRecordBytes()!), ['GET']);
    ...
    it('throws when there is no current context', () => {
      strictAssert.throws(() => appendAttributes(['v']), /no active thread context/);
    });
    ...
    it('silently drops entries past the 612-byte cap and sets the truncated flag', () => {
      ...
      runWithContext(() => {
        ...
        strictAssert.equal(decodeHeader(_currentRecordBytes()!).attrsDataSize, 514);
  ...
  describe('makeNamedContext', () => {
    ...
    describe('processContextAttributes', () => {
      it('matches the input keys plus the V8 layout constants', () => {
        ...
        strictAssert.equal(pca['threadlocal.nodejs_v1.wrapped_object_offset'], 24);
      ...
      it('is frozen and a defensive copy', () => {
        ...
        strictAssert.throws(() => {
          (pca as unknown as Record<string, string>)['threadlocal.schema_version'] =
            'tampered';
  ...
  describe('discovery contract', () => {
    it('exports otel_thread_ctx_nodejs_v1 as a TLS dynsym', function () {
      const addon = join(__dirname, '..', '..', 'build', 'Release', 'dd_pprof.node');
